...
╠══════════════════════════════════════════════════════════════════════════════╣
║  Versione: 5.0  |  19/09/2026                                                ║
║  Sostituisce e annulla la v4.0.                                              ║
║                                                                              ║
║  Modifiche rispetto alla v4 (decise dal nutrizionista il 19/09/2026):        ║
║   1. Icone dei pasti non piu' tagliate (la v1-v4 le ritagliava a destra      ║
║      e in basso). Stesso riquadro, stessa posizione.                         ║
║   2. Emoji dei box info (stella e divieto) rese come immagine, come le       ║
║      icone dei pasti: prima uscivano come quadratini.                        ║
║   3. Blocco "alternative" nativo per colazione e spuntino.                   ║
║   4. Segnaposto sul modello standard: 4 colazioni, spuntino delle 17,        ║
║      6 giorni strutturati + 1 libero.                                        ║
║  Font, colori, layout, header, footer, legenda, separatori: invariati.       ║
║                                                                              ║
║  ISTRUZIONI D'USO                                                            ║
║  ──────────────────────────────────────────────────────────────────────────  ║
║  1. Modificare SOLO "DATI PAZIENTE", "BOX_INFO", "COLAZIONI", "SPUNTINI"     ║
║     e "GIORNI"                                                               ║
║  2. NON toccare nulla sotto "═══ MOTORE GRAFICO ═══"                         ║
║  3. Lanciare: python3 TEMPLATE_PianoAlimentare_StudioTaglialatela_v5.py      ║
║     oppure:   python3 TEMPLATE..._v5.py output/nomefile.pdf                  ║
║                                                                              ║
║  STRUTTURA PASTO — 6 campi:                                                  ║
║  ("chiave_emoji", "LABEL", "Nome piatto", "dettaglio ingredienti",           ║
║   "~kcal", nota_o_None)                                                      ║
║                                                                              ║
║  PASTO CON ALTERNATIVE — il terzo campo e' una LISTA di alternative,         ║
║  dettaglio e kcal sono None:                                                 ║
║  ("colazione", "COLAZIONE", COLAZIONI, None, None, nota_o_None)              ║
║  Ogni alternativa: ("Nome", "dettaglio ingredienti", "~kcal", nota_o_None)   ║
║  Le lettere A, B, C... si assegnano da sole nell'ordine della lista.         ║
║                                                                              ║
║  chiave_emoji: "colazione" | "pranzo" | "spuntino" | "cena"                  ║
║  ATTENZIONE: lo SPUNTINO usa la chiave "spuntino", non "pranzo".             ║
║                                                                              ║
║  Nei testi di BOX_INFO le emoji stella e divieto diventano immagine da       ║
║  sole: si scrivono come carattere normale.                                   ║
║                                                                              ║
...
PAZIENTE = {
    ...
    "obiettivo":  "XX kg",
    "note_piano": "Struttura su 7 giorni (6 + 1 libero) · 4 pasti · versione 1",
    "mese":       "Mese Anno",
...
# ───────────────────────────────────────────────────────────────────
#  COLAZIONI E SPUNTINI DEL LIVELLO SCELTO
#  Uno solo il livello per tutto il piano (standard o rinforzata_calcio):
#  le stesse alternative valgono per tutti i giorni strutturati.
#  Ogni alternativa: ("Nome", "dettaglio ingredienti", "~kcal", nota_o_None)
# ───────────────────────────────────────────────────────────────────

COLAZIONI = [
    ("Colazione dolce 1",  "ingrediente 1 Xg · ingrediente 2 Xg · ingrediente 3 Xg", "~XXX kcal", None),
    ("Colazione dolce 2",  "ingrediente 1 Xg · ingrediente 2 Xg · ingrediente 3 Xg", "~XXX kcal", None),
    ("Colazione salata 1", "ingrediente 1 Xg · ingrediente 2 Xg · ingrediente 3 Xg", "~XXX kcal", None),
    ("Colazione salata 2", "ingrediente 1 Xg · ingrediente 2 Xg · ingrediente 3 Xg", "~XXX kcal", None),
]

SPUNTINI = [
    ("Spuntino 1", "ingrediente 1 Xg · ingrediente 2 Xg", "~XXX kcal", None),
    ("Spuntino 2", "ingrediente 1 Xg · ingrediente 2 Xg", "~XXX kcal", None),
]
...
#  Pasto con alternative: (chiave_emoji, "LABEL", LISTA, None, None, nota)
#
#  chiave_emoji: "colazione" | "pranzo" | "spuntino" | "cena"
...
def _giorno_strutturato(num, nome):
    """Segnaposto di un giorno strutturato. Nel piano reale i giorni si
    scrivono per esteso: pranzo e cena cambiano ogni giorno."""
    return {
        "num": num, "nome": nome,
        "pasti": [
            ("colazione", "COLAZIONE — ore 08:00", COLAZIONI, None, None, None),
            ("pranzo", "PRANZO — ore 13:00",
             "Nome piatto pranzo",
             "ingrediente 1 Xg · ingrediente 2 Xg · verdura Xg · olio EVO Xg",
             "~XXX kcal", None),
            ("spuntino", "SPUNTINO — ore 17:00", SPUNTINI, None, None, None),
            ("cena", "CENA — ore 20:00",
             "Nome piatto cena",
             "ingrediente 1 Xg · verdura Xg · olio EVO Xg",
             "~XXX kcal", None),
        ],
        "totale": "Totale giornata: ~X.XXX–X.XXX kcal secondo la colazione e lo spuntino scelti",
    }

GIORNI = [
    _giorno_strutturato("1", "Lunedì"),
    _giorno_strutturato("2", "Martedì"),
    _giorno_strutturato("3", "Mercoledì"),
    _giorno_strutturato("4", "Giovedì"),
    _giorno_strutturato("5", "Venerdì"),
    _giorno_strutturato("6", "Sabato"),
    {
        "num": "7", "nome": "Domenica — Giorno libero",
        "libero": True,
        "pasti": [
            ("colazione", "COLAZIONE",
             "A piacere",
            ...
            ("pranzo", "PRANZO",
             ...
             "~XXX kcal", None),
            ("spuntino", "SPUNTINO",
             "Indicazioni spuntino",
             "indicazioni generali spuntino giorno libero",
             "~XXX kcal", None),
            ("cena", "CENA",
             "Pasto libero",
...
def _make_emoji_imgs():
    ...
    mapping = {
        ...
        "spuntino":  "🥨",
        "cena":      "🌙",
        "stella":    "⭐",   # v5: box target
        "divieto":   "🚫",   # v5: box allergie
    }
    tmpdir = tempfile.mkdtemp()
    paths = {}
    for key, emoji in mapping.items():
        # v5: tela ampia, poi ritaglio sul disegno e riquadro quadrato
        # centrato. Nella v1-v4 la tela era 109x109 e il glifo (~125 px)
        # veniva tagliato a destra e in basso.
        big = PILImage.new("RGBA", (160, 160), (255, 255, 255, 0))
        ImageDraw.Draw(big).text((0, 0), emoji, font=font, embedded_color=True)
        big = big.crop(big.getbbox())
        lato = max(big.size)
        img = PILImage.new("RGBA", (lato, lato), (255, 255, 255, 0))
        img.paste(big, ((lato - big.width) // 2, (lato - big.height) // 2))
...
# ── Blocco pasto con alternative (v5) ──
def _meal_alt_block(emoji_path, label, alternative, nota):
    """Label pasto + "A scelta tra:" + una riga compatta per alternativa.
    Usa solo stili e colori gia' esistenti."""
    items = [_MealLabel(emoji_path, label)]
    items.append(Paragraph('<i>A scelta tra:</i>', _sMealNote))
    for i, (nome, det, kcal, nota_alt) in enumerate(alternative):
        lettera = "ABCDEFGH"[i]
        txt = f'<font color="#C0622B"><b>{lettera}</b></font>  <b>{nome}</b>'
        if det:
            txt += f' <font color="#444444">— {det}</font>'
        if kcal:
            txt += f'  <font color="#C0622B"><i>({kcal})</i></font>'
        items.append(Spacer(1, 1.2*mm))
        items.append(Paragraph(txt, _sMealTxt))
        if nota_alt:
            items.append(Paragraph(
                f'<font color="#2E7D8C">■</font>  <i>{nota_alt}</i>', _sMealNote))
    if nota:
        items.append(Spacer(1, 1.2*mm))
        items.append(Paragraph(
            f'<font color="#2E7D8C">■</font>  <i>{nota}</i>', _sMealNote))
...
# ── Emoji dei box info come immagine (v5) ──
def _box_txt(testo, emoji_paths):
    sz = 3.2*mm
    for car, key in (("⭐", "stella"), ("🚫", "divieto")):
        testo = testo.replace(
            car, f'<img src="{emoji_paths[key]}" width="{sz}" height="{sz}" valign="-2"/>')
    return testo
...
def _day_block(g, emoji_paths):
    ...
    for (ek, label, nome, det, kcal, nota) in g["pasti"]:
        if isinstance(nome, (list, tuple)):
            elems += _meal_alt_block(emoji_paths[ek], label, nome, nota)
        else:
...
def _cover(p, box_info, emoji_paths):
    ...
    # Box info
    tgt_data = [[
        Paragraph(_box_txt(box_info["target"],   emoji_paths), _sTgt),
        Paragraph(_box_txt(box_info["allergie"], emoji_paths), _sAlg),
    ]]
