...
Ultima modifica: 19/08/2026 · Dipendenze: MB, MD · Rilievi R33, R35, R37
...
**(f) Il vincolo si legge prima di produrre le alternative.** *(R33,
17/08/2026.)* Vale per il corpo e **per ogni elemento accessorio** — nota
biografica, occhiello, sommario, didascalia, abstract — che ha un limite proprio
in MD.

Il fatto che ha prodotto la regola: il 17/08 tre varianti di nota biografica
sono state scritte e misurate, e solo dopo si è riletto MD FORMATO 2, che fissa
300 battute con limite operativo 285. **Tutte e tre sforavano.** Il limite era
scritto correttamente nel modulo: il difetto non è stato di misura né di stima,
è stato di ordine di esecuzione.

Ordine obbligatorio, in tre mosse: si apre MD e si legge il limite operativo
dell'elemento → si scrivono le alternative dentro quel numero → si misura. Una
variante fuori vincolo non è una variante: è lavoro da rifare, e presentata
insieme alle altre sposta il giudizio su quale sia la migliore invece che su
quale sia ammissibile.

### 7.2 Regole di stesura

- Ogni affermazione tecnica porta `[Fxxx]` in linea, scritto contestualmente
- **La stesura procede una sezione alla volta**, avendo davanti le sole fonti
  mappate su quella sezione al PASSO 5 e il loro `conflitto_dichiarato`. Non si
  genera il corpo intero in un blocco unico. Il motivo è documentato dalla
  casistica di B1: i marcatori si perdono nei raccordi e nelle aperture di
  sezione, cioè esattamente dove il contesto è più largo e le fonti disponibili
  più numerose di quelle pertinenti
...
**VERIFICA DI VALIDITÀ CORRENTE** *(R37, 19/08/2026)*
R35 controlla **quando** una fonte è stata scritta. Non controlla se è ancora
valida. Sono due difetti diversi: una fonte del 2024 può essere stata ritirata,
un RCP del 2025 può essere stato revisionato, un principio attivo può aver
ricevuto una Nota Informativa Importante dopo la consultazione. Nessun passo
intercettava questo.

**Ambito ristretto.** Il controllo si esegue solo sulle fonti che reggono
**affermazioni portanti** — quelle su cui poggia un comportamento consigliato al
lettore — e non su tutto il registro. Sulle altre il costo supererebbe il
beneficio.

Tre verifiche, per tipo di fonte:
1. **Articoli**: controllo di ritiro o correzione (PubMed segnala *Retracted
   publication* ed *Erratum* sulla voce stessa).
2. **RCP**: data dell'ultima revisione dello stampato del prodotto dispensato,
   non della classe. Se la data è più recente di `data_consultazione`, la fonte
   si riapre.
3. **Principi attivi citati**: presenza di Note Informative Importanti
   successive alla consultazione.

**Si dichiara come consegna manuale, e questo fa parte della regola.** La banca
dati AIFA è un'applicazione lato client e `salute.gov.it` blocca il fetch
automatico: il motore non può eseguire queste tre verifiche da solo. Il PASSO 8
produce quindi l'**elenco puntuale di cosa va guardato** — quali fonti, quali
prodotti, quali principi attivi — e il lavoro non si chiude finché l'esito non
rientra. Una regola scritta come se la verifica fosse automatica fallirebbe in
silenzio, che è il modo di rompersi peggiore.

L'esito si registra aggiornando `data_consultazione` sulle fonti ricontrollate:
il campo diventa così la prova che il controllo è stato fatto, e non una data di
primo incontro.
